...
export class FeedProviderService {
  ...
  async uploadFeedItem(caption: string, description: string,file: File): Promise<any> {
    const res = await this.api.upload('/feed', file, {caption: caption ,url: file.name,description: description});
